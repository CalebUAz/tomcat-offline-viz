...
class Window(QWidget):
    ...
    def changedValue(self, value):
        ...
        x, y, point_scale, id_labels = read_pupil_data()
        fig, ax = plt.subplots()
        polyline = ax.plot(x[value], y[value], "C3", lw=2)
        points = ax.scatter(x[value], y[value], s=point_scale[value], alpha=0.2)
        ax.imshow(rgb_image)

        canvas = FigureCanvas(fig)
        canvas.setGeometry(60, 60, 1280, 720)
        canvas.show()
